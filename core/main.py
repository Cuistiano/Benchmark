...
from hdf5reader import DummyH5Dataset
if not config.use_fundamental:
    from hdf5reader import collate_fn_E as collate_fn
else:
    from hdf5reader import collate_fn_F as collate_fn
...
def main(config):
    ...
    if config.run_mode == "train":
        ...
        train_seqs = glob(config.data_tr+'/*')
        train_datasets = [DummyH5Dataset(seq, config.use_fundamental) for seq in train_seqs]
        ...
        valid_seqs = glob(config.data_va+'/*')
        valid_datasets = [DummyH5Dataset(seq, config.use_fundamental) for seq in valid_seqs]
