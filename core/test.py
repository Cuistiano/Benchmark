...
from evaluation import eval_nondecompose, eval_decompose, eval_F_wo_RANSAC, eval_F_w_RANSAC, compute_metric
from utils import tocuda, get_pool_result


def test_sample(args):
    _xs, _dR, _dt, _e_hat, _y_hat, _y_gt, _K1, _K2, config = args
    ...
    if _K1 is not None:
        _K1, _K2 = _K1.astype('float64'), _K2.astype('float64')
    ...
    if not config.use_fundamental:
        ...
        if not config.use_ransac:
            _err_q, _err_t, _, _, _num_inlier, _mask_updated, _R_hat, _t_hat = \
                eval_F_wo_RANSAC(_x1, _x2, e_hat_out, _dR, _dt, _K1, _K2, _y_hat_out)
        else:
            _err_q, _err_t, _, _, _num_inlier, _mask_updated, _R_hat, _t_hat = \
                eval_F_w_RANSAC(_x1, _x2, _dR, _dt, _K1, _K2, mask=_mask_before, ransac_th=config.ransac_th, ransac_iter=config.ransac_iter)
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            if config.use_fundamental:
                ...
                #e_hat = e_hat / torch.norm(e_hat.reshape(-1,9), dim=1, keepdim=True)
        ...
        if len(pool_arg) > 0:
            results += get_pool_result(num_processor, test_sample, pool_arg)
        #print('net time '+str(model_time/len(loader_iter)))
...
def test(data_loader, model, config):
    ...
    if not os.path.exists(save_file_best):
        ...
        exit(1)
    # Restore model
    checkpoint = torch.load(save_file_best, map_location=torch.device('cuda') if not config.use_cpu else torch.device('cpu'))
